...
class Scanner:
    ...
    def evaluate_pair(
        ...
        # БАЗОВЫЙ критерий прибыльности (всегда, не отключается): сырой спред
        # должен перекрывать round-trip комиссии — вход И выход на ОБЕИХ биржах,
        # иначе комиссия съест профит.
        if raw <= fee_cost:
            reasons.append(f"комиссии({fee_cost*100:.3f}%) съедают спред")
        # Фильтр 2: чистый порог со всеми издержками (опц.)
